...
>
> [FMC講座 資料 - uesyuu](https://drive.google.com/file/d/1Ud6nbN8iJQNaSVKaRufsMItI2QZKtJtf/view)
>
> [DR初心者が回答を書けるようになるためには - uesyuu's Blog](https://blog.uesyuu.com/post/2025-01-21-how-to-write-dr-solution-for-beginner/)
...
NISSトレースとは、ある特定の4つのコーナーに対してそれぞれNISSした後にどの位置関係になるかを予測する手法です。

4a, 4bの場合、bad corner4つに対してNISSトレースを行います。

NISSトレースは説明が難しいのですが、ここではとても簡単なNISSトレースの判断方法をお教えします。
...
NISSトレースについて簡潔にまとめると以下の通りになります。

- 4つのコーナーの各対面色の色配置の中に3個/1個の色配置が1つでもあった場合、NISSトレースの結果は4b
- 4つのコーナーの各対面色の色配置の中に3個/1個の色配置が1つもなかった場合、NISSトレースの結果は4a
...
0cの判断方法では、先ほど説明したNISSトレースを使います。

0cの判断方法は以下の通りです。

- U面の4コーナー、F面の4コーナー、R面の4コーナーに対してそれぞれNISSトレースを行う
- NISSトレースの各結果に基づいて以下のように判別できる
  - NISSトレースの結果が3つとも4a: 0c0
  - NISSトレースの結果が4aが1つ、4bが2つ: 0c3
  - NISSトレースの結果が3つとも4b: 0c4

具体例を1つ出します。

`スクランブル: R' U' F D F2 D' B2 D' B2 D2 B2 D' L2 U' R2 F' U2 F' D B R' D2 F R' U' F`

bad cornerは1つもないため、これは0cのsubsetです。

U面の4コーナーの色配置の中には、白が1個と黄色が3個あるためNISSトレースの結果は4bです。

F面の4コーナーの色配置の中には、青が1個と緑が3個あるためNISSトレースの結果は4bです。

R面の4コーナーの色配置の中には、3個1個の色配置のものはなく全て2個2個の配置のため、NISSトレースの結果は4aです。

4aが1つ、4bが2つなのでこのsubsetは0c3と判別できます。

&nbsp;

#### 2cの判別方法

2cの判断方法でも、0cと同じくNISSトレースを使います。

2cの判断方法は以下の通りです。

- 2つのbad cornerを隣接させた配置にする
- bad cornerが存在しない2面についてNISSトレースを行うことで判別ができる
  - 両面とも4a: 2c5
  - 片面が4a, もう片面が4b: 2c4
  - 両面とも4b: 2c3か2c4
    - 両面とも4bの場合は、bad cornerが2つある2面それぞれのgood corner2つ、計4コーナーをNISSトレースして判別する
      - 結果が4a: 2c4
      - 結果が4b: 2c3

ちょっとわかりにくいかもしれないので、具体例を出します。

`スクランブル: R' U' F L2 F' R2 F L2 F L2 F2 L2 U2 F' L' F D2 F2 U R2 F' R2 U' R' U' F`

bad cornerはUBLとDFLの計2つあるため、これは2cのsubsetです。

この2cの配置はコーナーが隣接しておらず離れているので、R2で近づけるかもしくはU2で近づけます。今回はR2で近づけてみましょう。

この状態でbad cornerが存在しない2面はそれぞれU面とB面です。U面の4コーナーをNISSトレースすると、白が1個と黄色が3個あったのでNISSトレースの結果は4bです。B面の4コーナーをNISSトレースすると、緑が1個と青が3個あったのでNISSトレースの結果は4bです。

結果が両面とも4bだったので、もう1ステップ必要になります。

今bad cornerはDFLとDFRにあります。bad cornerを2つとも含む面はF面とD面です。F面について、bad cornerではないコーナー(good corner)はUFRとUFLです。D面について、bad cornerではないコーナー(good corner)はDBRとDBLです。この4コーナーをNISSトレースします。

NISSトレースしてみると、緑が1個と青が3個ありました。そのため結果は4bです。

ということで、3つとも全て4bだったので、この2cのsubsetは2c3だと判別できます。
...
もしエッジがいくつか揃っていない場合、一度QTを回して4a1 4c4eにしてから解くといいでしょう。
...
4c4eであればセットアップして解くことが可能です。R2 F2 R2 UなどのHTRトリガーが有用です。

4c4eでない場合、確実に+2qt必要になるので一度QTを回して4b2か4a2に持ち込んでから取り組みましょう。
...
4a1にする時ちゃんと4c4eにしないと1qtで解けないことに注意！

&nbsp;

### 4b2

4c2eならU' R2 U、4c4eならU R2 F2 UといったHTRトリガーが使えます。
...
2cなので、bad cornerの存在しない2面のコーナーをそれぞれNISSトレースします。今回はF面とL面ですね。

- F面: オレンジが1個、赤が3個→4b
- L面: 黄色が1個、白が3個→4b

両面とも4bだったので次のステップに進みます。

bad cornerが2つある面はR面とB面で、それぞれの面におけるgood cornerはUFR, DFR, UBL, DBLです。これら4コーナーをNISSトレースすると、オレンジが1個と赤が3個なので、結果は4bです。

3個とも4bだったので、このDRの状態は2c3とわかりました。

2c3なので対面にbad cornerを置いてQTを回します。
この場合すでに対面にbad cornerがあるので、Dを回すことで4b2にします。
