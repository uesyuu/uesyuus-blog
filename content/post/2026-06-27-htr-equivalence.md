...
の場合、完成状態から[B' C]を回した状態ががHTR状態なら①と②はHTReq
```
...
バリエーションのどちらか片方でスイッチし、[F2 B2 U]でqtを減らして4a1 4eにして更にバリエーション側にスイッチすると、[B R2 B]と[F U2 F]の2バリエーションそれぞれで異なる4a1 4eが得られます。今回の場合、[B L2 B]であれば2手でHTRが完成する形でした。
